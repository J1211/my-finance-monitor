...
# --- 1. 界面配置 ---
st.set_page_config(page_title="GSMI Tactical | 宏观精密验证版", layout="wide")
...
try:
    df_main = fetch_and_sync_data()
    df = calculate_history(df_main, fms_cash)
    latest = df.iloc[-1]
...
st.caption("GSMI Tactical | 45% 流动性 + 25% 现实 + 15% 情绪 + 15% 汇率。数据由 FRED & yfinance 提供。")

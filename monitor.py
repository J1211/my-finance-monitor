...
st.markdown("""
    ...
    .quadrant-box { padding: 10px; border-radius: 5px; border: 1px solid #333; background-color: #1a1c24; margin-bottom: 10px; }
    </style>
...
def fetch_macro_data():
    ...
    btc = safe_get_yf("BTC-USD") # 新增：比特币观察
...
try:
    ...
    s_nl_pos = 15 if curr_nl > ma4_nl else 0
    s_nl_slope = 10 if curr_nl > prev_nl else 0
    s_nl = s_nl_pos + s_nl_slope
    ...
    st.markdown("---")
    tabs = st.tabs(["💧 流动性水源 (净流动性+BTC)", "🧠 情绪/购买力", "🏗️ 经济/现实", "📉 执行确认"])
    ...
    with tabs[0]:
        ...
        col_q1.markdown('<div class="quadrant-box"><b>🔵 25分: 扩张期</b><br>水位高+持续放水<br>🚀 <b>进攻</b></div>', unsafe_allow_html=True)
        col_q2.markdown('<div class="quadrant-box"><b>🟡 15分: 滞涨期</b><br>水位高+水龙头关小<br>⚠️ <b>警惕</b></div>', unsafe_allow_html=True)
        col_q3.markdown('<div class="quadrant-box"><b>🟠 10分: 修复期</b><br>水位低+水龙头开启<br>🔍 <b>观察</b></div>', unsafe_allow_html=True)
        col_q4.markdown('<div class="quadrant-box"><b>🔴 0分: 衰退期</b><br>水位低+持续漏水<br>🛑 <b>空仓</b></div>', unsafe_allow_html=True)

        m1, m2, m3 = st.columns(3)
        m1.metric("净流动性 (NL)", f"${curr_nl:.2f}T", f"得分: {s_nl}/25")
        m2.metric("比特币 (BTC) - 金丝雀", f"${get_val(btc_ser)/1000:.1f}k", f"周变化: {(get_val(btc_ser)/get_val(btc_ser, -6)-1)*100:+.1f}%")
        m3.metric("逆回购 (RRP)", f"${df_fed['rrp'].iloc[-1]/1000000:.2f}T", "流动性缓冲垫")

        # 对比图表：NL vs BTC
        fig_nl_btc = go.Figure()
        fig_nl_btc.add_trace(go.Scatter(x=df_fed.index, y=df_fed['net_liquidity'], name="净流动性(T)", line=dict(color='#00ffcc', width=3)))
        fig_nl_btc.add_trace(go.Scatter(x=btc_ser.index, y=btc_ser.values/10000, name="BTC价格 (x10k)", line=dict(color='#ffd700', dash='dot'), yaxis="y2"))
        fig_nl_btc.update_layout(height=400, template="plotly_dark", hovermode="x unified",
                                 legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
                                 yaxis=dict(title="净流动性 (万亿)", side="left"),
                                 yaxis2=dict(title="BTC (x10k)", side="right", overlaying="y", showgrid=False))
        st.plotly_chart(fig_nl_btc, use_container_width=True)
    ...
    with tabs[3]:
        ...
        if not as300_ser.empty and not hsi_ser.empty:
            ...
            st.write(f"📊 相对强度 HSI vs AS300: {float(norm.iloc[-1,1]-norm.iloc[-1,0]):+.2f}%")
...
st.caption("GSMI Tactical Version | 数据驱动决策，逻辑胜过直觉。")
